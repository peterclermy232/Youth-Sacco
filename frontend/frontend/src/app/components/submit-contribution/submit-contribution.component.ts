...
@Component({
  ...
  styleUrls: ['./submit-contribution.component.scss']
})
export class SubmitContributionComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    this.contributionService.createContribution(this.contributionForm.value).subscribe({
      next: (contribution) => {
        this.snackBar.open('Contribution submitted successfully! Awaiting admin verification.', 'Close', {
          duration: 5000
        });
      ...
      error: (error) => {
        ...
        const errorMessage = error.error?.mpesa_transaction_code?.[0] ||
                           error.error?.detail ||
